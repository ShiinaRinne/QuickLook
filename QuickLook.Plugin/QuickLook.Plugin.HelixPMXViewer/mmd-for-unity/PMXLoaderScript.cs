...
namespace MMDExtensions
{
    ...
    public partial class PMXLoaderScript
    {
        ...
        private PMXFormat.Vertex ReadVertex()
        {
            ...
            result.pos = ReadSinglesToPoint3D(binary_reader_);
            result.normal_vec = ReadSinglesToPoint3D(binary_reader_);
            result.uv = ReadSinglesToPoint(binary_reader_);
            result.add_uv = new Point4D[format_.header.additionalUV];
            for (int i = 0; i < format_.header.additionalUV; i++)
            {
                result.add_uv[i] = ReadSinglesToPoint4D(binary_reader_);
            }
        ...
        private PMXFormat.BoneWeight ReadBoneWeightSDEF()
        {
            ...
            result.bone2_ref = CastIntRead(binary_reader_, format_.header.boneIndexSize);
            result.bone1_weight = binary_reader_.ReadSingle();
            result.c_value = ReadSinglesToPoint3D(binary_reader_);
            result.r0_value = ReadSinglesToPoint3D(binary_reader_);
            result.r1_value = ReadSinglesToPoint3D(binary_reader_);
            return result;
        }
        ...
        private PMXFormat.Bone ReadBone()
        {
            ...
            result.bone_position = ReadSinglesToPoint3D(binary_reader_);
            ...
            {
                // 座標オフセットで指定
                result.position_offset = ReadSinglesToPoint3D(binary_reader_);
            }
            else
            {
                // ボーンで指定
            ...
            {
                result.axis_vector = ReadSinglesToPoint3D(binary_reader_);
            }
            if ((result.bone_flag & PMXFormat.Bone.Flag.LocalAxis) != 0)
            {
                result.x_axis_vector = ReadSinglesToPoint3D(binary_reader_);
                result.z_axis_vector = ReadSinglesToPoint3D(binary_reader_);
            }
        ...
        private PMXFormat.IK_Link ReadIkLink()
        {
            ...
            {
                result.lower_limit = ReadSinglesToPoint3D(binary_reader_);
                result.upper_limit = ReadSinglesToPoint3D(binary_reader_);
            }
            return result;
        }
        ...
        private PMXFormat.MorphOffset ReadBoneMorphOffset()
        {
            ...
            result.move_value = ReadSinglesToPoint3D(binary_reader_);
        ...
        private PMXFormat.MorphOffset ReadUVMorphOffset()
        {
            ...
            result.vertex_index = CastIntRead(binary_reader_, format_.header.vertexIndexSize);
            result.uv_offset = ReadSinglesToPoint4D(binary_reader_);
            return result;
        }
        ...
        private PMXFormat.MorphOffset ReadImpulseMorphOffset()
        {
            ...
            result.move_velocity = ReadSinglesToPoint3D(binary_reader_);
            result.rotation_torque = ReadSinglesToPoint3D(binary_reader_);
            return result;
        }
        ...
        private PMXFormat.Rigidbody ReadRigidbody()
        {
            ...
            result.shape_size = ReadSinglesToPoint3D(binary_reader_);
            result.collider_position = ReadSinglesToPoint3D(binary_reader_);
            result.collider_rotation = ReadSinglesToPoint3D(binary_reader_);
        ...
        private PMXFormat.Joint ReadJoint()
        {
            ...
            switch (result.operation_type)
            {
                case PMXFormat.Joint.OperationType.Spring6DOF:
                    ...
                    result.position = ReadSinglesToPoint3D(binary_reader_);
                    result.rotation = ReadSinglesToPoint3D(binary_reader_);
                    result.constrain_pos_lower = ReadSinglesToPoint3D(binary_reader_);
                    result.constrain_pos_upper = ReadSinglesToPoint3D(binary_reader_);
                    result.constrain_rot_lower = ReadSinglesToPoint3D(binary_reader_);
                    result.constrain_rot_upper = ReadSinglesToPoint3D(binary_reader_);
                    result.spring_position = ReadSinglesToPoint3D(binary_reader_);
                    result.spring_rotation = ReadSinglesToPoint3D(binary_reader_);
                    break;
                default:
                    //empty.
                    break;
        ...
        private static Point4D ReadSinglesToPoint4D(BinaryReader binary_reader_)
        {
            const int count = 4;
        ...
        private static Point3D ReadSinglesToPoint3D(BinaryReader binary_reader_)
        {
            const int count = 3;
            float[] result = new float[count];
            for (int i = 0; i < count; i++)
